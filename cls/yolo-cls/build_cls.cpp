...
int main() {
    // 改改路径
    const std::string wts_name = "/home/rc/yolo11trt/cls/yolo-cls/models/best_cls.wts";    // 在这里设置你的 .wts 文件路径
    const std::string engine_name = "/home/rc/yolo11trt/cls/yolo-cls/models/best_cls.engine"; // 在这里设置输出 engine 文件路径
    ...
    IHostMemory* serialized_engine = buildEngineYolo11Cls(builder, config, DataType::kFLOAT, wts_name, gd, gw,
                                                         model_suffix, max_channels);
    if (!serialized_engine) {
        std::cerr << "构建引擎失败:( buildEngineYolo11Cls 返回空" << std::endl;
        delete config;
        delete builder;
        return -1;
    }
